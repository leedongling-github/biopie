...
if __name__ == '__main__':
    ...
    options = OptionConfig(model_name='Bert', batch_size=5, epochs=1,
                           save_epoch_model=1, sele_best_metric='F1', max_seq_len=128,
                           valid_ratio=0.1, verbose=3, pred_test_ent=0,
                           fold_num=10, fold_num_run=1,
                           add_loss_model=0, cnn_kernel_size=[3, 4, 5],
                           calc_docu_metric=0, pred_file_fold=1,
                           bld_ent_types=elist, bld_ent_mode=1,
                           diff_ent_type=0, mark_ent_pair='##@@',  # why ##@@, not #@#@?
                           case_sensitive=0, sent_simplify=0)
    #
    options.add_option('--sbel_decompose', dest='sbel_decompose', default=0, help='decompose SBEL task')
    ...
    options.add_option('--sbel_merge', dest='sbel_merge', default=1, help='merge SBEL after prediction')
