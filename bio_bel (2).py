...
# allocate GPU memory only if necessary
...
def evaluate_bel_statements(tcfg, wdir, cpsfile, folds=None):
    """
    评估语料文件中的BEL语句在各个层面上的预测情况, *.sbel vs *.bel.[eval_bel_suff]
    如果pred_file_fold设置，则对各卷的sbel.prd.f0进行BEL语句转换和评估
    :param wdir, cpsfile, folds:
    """
    ...
    efilename = '{}/{}.ent{}'.format(wdir, cpsfile, '.prd' if tcfg.pred_test_ent else '')
    belset.load_entity_mentions(tcfg.verbose, efilename)
    # merge SBEL to BEL statements w/o functions
    # if not folds:  folds = [None]
    for fold in folds:
        fsuf = '.f{}'.format(fold) if tcfg.pred_file_fold else ''
        ...
        # merge SBEL statements from belset.test_sbels to BEL statements w/o functions ,
...
def browse_bel_statements(tcfg, wdir, cpsfile, fold):
    ...
    如果是prd预测文件且预测结果带有卷标，则将带卷标文件拷贝到无卷标文件
    :param tcfg:
    :param wdir:
    :param cpsfile:
    :param fold:
    ...
    if tcfg.eval_bel_suff == 'prd' and tcfg.pred_file_fold:
...
def review_sbel_corpus(op, tcfg, task, wdir, cpsfiles, cpsfmts):
    ...
    dcfg = sbelsets.dcfg
    sbelsets.output_docsets_instance_statistics(dcfg.rel_typedict, rcounts, 'Relations', logfile='sBelStatements.cnt')
    sbelsets.output_docsets_instance_statistics(dcfg.func_typedict, fcounts, 'Functions', logfile='sBelStatements.cnt')
    ...
    sbelsets.output_docsets_instance_statistics(dcfg.ent_typedict, ccounts, 'Entity Mentions', levels,
    ...
    if tcfg.sbel_decompose:
        ...
        bresets.output_docsets_instance_statistics(bresets.dcfg.rel_typedict, ccounts, 'Relation Mentions',
                                                   logfile='brelations.cnt')
        # save to test_sbel_re_insts.txt
        bfileset = bresets.filesets[0]
        tfilename = '{}/{}_{}_inst.txt'.format(wdir, bresets.ftask, bfileset.id)
        bfileset.print_docset_instances(filename=tfilename, level='inst', verbose=tcfg.verbose)
        ...
        uresets.output_docsets_instance_statistics(uresets.dcfg.rel_typedict, ccounts, 'uRelation Mentions',
                                                   logfile='urelations.cnt')
        # save to test_sbel_ure_insts.txt
        ufileset = uresets.filesets[0]
        tfilename = '{}/{}_{}_inst.txt'.format(wdir, uresets.ftask, ufileset.id)
        ufileset.print_docset_instances(filename=tfilename, level='inst', verbose=tcfg.verbose)
    return
...
def train_eval_corpus(op, tcfg, task, wdir, cpsfiles, cpsfmts, bepos=None, seed=0, folds=None):
    ...
    if tcfg.sbel_decompose:
        ...
        if tcfg.sbel_unary:
            # tcfg.class_weight = np.array([1, 1, 1, 1, 1, 1, 5])
...
if __name__ == '__main__':
    ...
    # class_weight = np.array([1, 1, 1, 1, 1, 1, 5])
    class_weight = None
    options = OptionConfig(model_name='Bert', batch_size=2, sepo=0, epochs=3,
                           save_epoch_model=1, sele_best_metric='F1', max_seq_len=100,
                           valid_ratio=0, verbose=3, pred_test_ent=0, class_weight=class_weight,
                           fold_num=10, fold_num_run=1,
                           add_loss_model=0, cnn_kernel_size=[3, 4, 5],
                           calc_docu_metric=0, pred_file_fold=1,
                           bld_ent_types=elist, bld_ent_mode=1, mask_nest_ent=1,
                           diff_ent_type=0, mark_ent_pair='##@@',  # why ##@@, not #@#@?
                           case_sensitive=0, sent_simplify=1)
    #
    options.add_option('--sbel_decompose', dest='sbel_decompose', default=1, help='decompose SBEL task')
    options.add_option('--sbel_binary', dest='sbel_binary', default=1, help='binary RE for SBEL')
    options.add_option('--sbel_unary', dest='sbel_unary', default=0, help='unary RE for SBEL')
    options.add_option('--sbel_merge', dest='sbel_merge', default=0, help='merge SBEL after prediction')
    # eval_bel_suff[gold, prd]: gold-recast gold BEL, prd-predicted BEL,
    ...
    # SBEL
    main('tvp', 'sbel', 'BEL', ('train', 'test'), 'ss', tcfg, bepos=[-1, -1], seed=0, folds=range(1))  # SBEL
    ...
    PS: 在导入外部的rel或urel预测文件(test.rel.prd.f0/test.urel.prd.f0)时，应将文件名拷贝为test_sbel.rel.prd.f0等，
        ...
        然后运行vp功能合并成SBEL语句，再用e功能评估BEL性能。   
    """
